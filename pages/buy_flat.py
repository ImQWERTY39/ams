...
def page(root: tk.Tk):
    ...
    tools.create_button(frame, text="Submit", command=lambda: own_flat(root, frame, flat_number.get())).place(x=670, y=525)
...
def own_flat(root, frame, flat_number):
    ...
    if not flat[1]: tk.messagebox.showerror("Flat not available", f"Flat {flat_number} is not for sale"); return
...
def rent_out(root, flat):
    ...
    tools.create_button(frame, text="Submit", command=lambda: rent(root, frame, flat[0], tenant_name.get())).place(x=670, y=525)
    tools.create_button(frame, text="Quit", command=lambda: tools.switch_frame(root, frame, pages.dashboard.page)).place(x=525, y=525)
...
def for_buy(root, flat):
    ...
    tools.create_button(frame, text="Submit", command=lambda: buy(root, frame, flat[0], owner_name.get(), phno.get(), email.get())).place(x=670, y=525)
    tools.create_button(frame, text="Quit", command=lambda: tools.switch_frame(root, frame, pages.dashboard.page)).place(x=525, y=525)

def rent(root, frame, flat_number, tenant_name):
    if database.rent_out_flat(flat_number, tenant_name) == 1: tk.messagebox.showerror("Invalid detail", "Tenant name is empty")
    else:
