...
def page(root: tk.Tk):
    ...
    tools.create_button(frame, text="Submit", command=lambda: check_delete(root, frame, flat_number.get())).place(x=670, y=525)
...
def delete(root, flat):
    ...
    tools.create_button(frame, text="Delete", command=lambda: perf_delete(root, frame, flat)).place(x=670, y=525)
    tools.create_button(frame, text="Quit", command=lambda: tools.switch_frame(root, frame, pages.dashboard.page)).place(x=525, y=525)

def perf_delete(root, frame, flat):
    if not tk.messagebox.askyesno("Confirmation", "Are you sure you want to sell the flat?\nThis is an irreserable change"): return
    deleted_owner = database.delete_flat(flat)
    if deleted_owner is not None: tk.messagebox.showinfo("Deleted Owner Information", f"Deleted {deleted_owner[0]}'s information'")
