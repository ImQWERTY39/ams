...
def try_modify(root, frame, flat_number):
    ...
    if flat is None: 
        ...
        tk.messagebox.showerror("Invalid details", "Only owned houses can be modified")
    else:
...
def modify(root, flat):
    ...
    (a_cb, availability) = tools.create_checkbox(frame, 250, 340, flat[1])
    (fr_cb, for_rent) = tools.create_checkbox(frame, 250, 405, flat[2])
    tenant_name = tools.create_entry(frame, x=250, y=475)

    tenant_name.config(state=("normal" if for_rent.get() and (not availability.get()) else "disabled"))
    a_cb.config(command = lambda: tenant_name.config(state=("normal" if for_rent.get() and (not availability.get()) else "disabled")))
    fr_cb.config(command = lambda: tenant_name.config(state=("normal" if for_rent.get() and (not availability.get()) else "disabled")))
...
def perf_modify(root, frame, flat, new_availability, new_for_rent, new_tenant):
    choice = tk.messagebox.askyesno(
        "Confirmation", "Are you sure you want to modify the details of this flat?\nThis is an irreserable change"
    )
    if not choice: return

    success = database.modify_flat(flat, new_availability, new_for_rent, new_tenant)
    if success == 1: tk.messagebox.showerror("Invalid detail", "Current options are for selling flat, consider going to 'Sell Flat' or recheck the selected options")
    elif success == 2: tk.messagebox.showerror("Invalid detail", "Tenant name is empty")
    else:
